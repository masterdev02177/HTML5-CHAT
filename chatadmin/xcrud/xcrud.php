...
class Xcrud
{
            ...
            if ($this->alert_create)
            {
                foreach ($this->alert_create as $alert)
                {
                    ...
                    if (Xcrud_config::$email_enable_html)
                        $message = $alert['message'] . '<br /><br />' . "\r\n" . ($alert['link'] ? '<a href="../../admin/xcrud/' . $alert['link'] .
    ...
    protected function _set_field_names()
    {
        foreach ($this->fields as $key => $field)
        {
            if (isset($this->labels[$key]))
                $this->fields_names[$key] = $this->html_safe2($this->labels[$key]) . (isset($this->validation_required[$key]) ? '&#42;' :
                        '');
            else
                $this->fields_names[$key] = $this->html_safe2($this->_humanize($field['field'])) . (isset($this->validation_required[$key]) ?
                        '&#42;' : '');
        }
    }
    ...
    protected function create_bool($name, $value = '', $tag = array())
    {
        ...
        return $this->open_tag('div', $this->theme_config('checkbox_container')) . $this->open_tag('label') . $this->single_tag($tag,
            $this->theme_config('bool_field'), $this->field_attr[$name], true) . $this->open_tag('span') . $this->close_tag('span') . $this->close_tag('label') . $this->close_tag('div');;
    }
    ...
    protected static function error($text = 'Error!')
    {
        ...
        exit('<div class="xcrud-error" style="position:relative;line-height:1.25;padding:15px;color:#BA0303;margin:10px;border:1px solid #BA0303;border-radius:4px;font-family:Arial,sans-serif;background:#FFB5B5;box-shadow:inset 0 0 80px #E58989;">
            <span style="position:absolute;font-size:10px;bottom:3px;right:5px;">Error</span>' . $text . '</div>');
    }
    ...
    {
        if ($this->links_label)
            return preg_replace('/(http:\/\/|https:\/\/)([^\s]+)/u', '<a target="_blank" href="../../admin/xcrud/$1$2">' . $this->links_label['text'] .
                '</a>', $txt);
        else
            return preg_replace('/(http:\/\/|https:\/\/)([^\s]+)/u', '<a target="_blank" href="../../admin/xcrud/$1$2">$1$2</a>', $txt);
    }
    ...
    protected function html_safe2($text)
    {
        return $text;
