...
function convertIP($value, $fieldname, $primary_key, $row, $xcrud)
{
...
?>
    <div>
        <label for="searchProperty">Search propetry</label>
        <input type="text" class="xcrud-input form-control" id="searchProperty" placeholder="Enter property to search ex: kick" style="max-width: 300px">
    </div>
    <script>
        ...
        function injectJS() {
            ...
            $(document).on('keyup', '#searchProperty', function() {
                ...
                if (value) {
                    $('div.form-group').hide();
                    var temp = `div.form-group:contains(${value})`;
                    $(temp).show();
                }
            })
        }
