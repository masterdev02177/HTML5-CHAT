...
/*else {
	...
	bootstrap_modal: function(header, content) {
		...
		jQuery("#xcrud-modal-window .modal-content").html('<div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button><h4 class="modal-title">' + header + '</h4></div>');
	...
	init_tooltips: function(container) {
		if (jQuery(container).find('.xcrud-tooltip').size()) {
			jQuery(container).find('.xcrud-tooltip').tooltip({html:true});
		}
	},
		...
		$(".xcrud").on("click", ".xcrud-action", function() {
			...
			if (confirm_text && !window.confirm(confirm_text)) {
				...
				if ($(this).hasClass('xcrud-in-new-window')) {
					...
					if (data.task == 'save') {
						var flag = false;
						var inputs = $('input[required="true"], textarea[required="true"]');
