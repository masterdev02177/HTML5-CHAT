...
var ChatHTML5 = function (room, config, traductions) {
    ...
    $('#smileyButton').click(function (_e) {
        $('#smileyContainer').slideToggle(100);
        $('#searchGifInput').focus();
    });


    $('#backgroundBtn').click(function (_e) {
    ...
    $(document).on('click', 'div.content img.gif', function (_e) {
    ...
    $('#uploadImageFile').on('change', function (_e) {
    ...
    this.searchGif = function (_keyword, url, gifElement) {
        $('#smileyContentLoader').show();
        var xhr = $.get(url);
        function _loadimages(_imgArr, callback) {
            var imagesLoaded = 0;
            _loadAllImages(callback);
        }
        var images = Array();
    ...
    $('#forgottenBtn').click(function (_e) {
    ...
    $('#loginRegisterBtn').click(function (_e) {
        $('#registerModal').modal('show');
    });

    $('#roomPrivateRadio').click(function (_e) {
        $('#divPasswordRoomCreate').show();
    })
    $('#roomPublicRadio').click(function (_e) {
    ...
    this.start = function (myuser) {
        console.log('start visichatdesign.com HHHHHHHHHH');
    ...
    this.showUsersMenu = function () {
        ...
        for (var index in chatHTML5.users) {
            var user = chatHTML5.users[index];
            var el = sprintf('<div data-id="%s">%s</div>', user.id, user.username);
    ...
    this.addOrSelectPrivateChat = function (user, selected) {
        ...
        if (chatHTML5.config.chatType === 'window') {
            ...
            //tab
            var image = sprintf('<img src="%s" class="userTabAvatar">', (user || {}).image);
            var title = sprintf('%s %s <div class="unread" title="%s"></div>&nbsp;&nbsp;', image, username, chatHTML5.traductions.unreadMessages);
    ...
    $(document).on('tabChanged', function (_e, _room) {
    ...
    $(document).on('jspanelclosed', function closeHandler(_event, id) {
    ...
    this.sendText = function (request) {
        if(request == 0){
        ...
        if(!document.getElementById('biggerDiv').className.includes('fontSelected')){
            font_size = '15px';
        }
        var extras = {
            request: request,
            color: color, font_size: font_size, bold: $('#boldDiv').hasClass('fontSelected'), italic: $('#italicDiv').hasClass('fontSelected'),
            underline: $('#underlineDiv').hasClass('fontSelected'), bigger: $('#biggerDiv').hasClass('fontSelected'), date: Date.now()
        };
        if (currentTab.room) {
            ...
            if (destinataire && action == 'whisper') {
                ...
                chatHTML5.socket.emit('whisper', destinataire.id, plainText, extras, request);
                chatHTML5.receiveText(chatHTML5.myUser, plainText, extras, request);
            }
            else if (destinataire && action == 'mention') {
                extras.mention = 1;
                extras.userid = destinataire.id;
                chatHTML5.socket.emit('send', this.myUser, plainText, extras, request);
            }
            else {
                chatHTML5.socket.emit('send', this.myUser, plainText, extras, request);
            }
            ...
            plainText = chatHTML5.filterBadWord(plainText);
            chatHTML5.socket.emit('sendPrivate', this.myUser, plainText, currentTab.id, extras, request);
            var destinationUser = jQuery.extend(true, {}, chatHTML5.myUser);
            destinationUser.id = currentTab.id;
            chatHTML5.receivePrivate(destinationUser, plainText, extras, request);
        }
    ...
    this.getUserByUsername = function (username) {
        var resultat = false;
        $.each(chatHTML5.users, function (_id, user) {
    ...
    this.changeFontsize = function (font_size) {
        // $('div.tab-content .tab-pane.active').eq(0).empty();
        document.getElementsByClassName("tab-content")[0].style.font_size = font_size;
        console.log( document.getElementsByClassName("tab-content")[0].style.font_size);
    }
    this.receiveText = function (user, message, extras, request) {
        if(extras.request == 1){
            var webmasterid = this.webmasterid;
            console.log('webmasterid:', webmasterid);
            $.post(chatHTML5.config.ajax, { a: 'deleteAllMessages', webmasterid: webmasterid }, function () {
                ...
                <div class="message flex-property %s %s">\
                    <img src="%s" alt="%s" class="userItem" >\
                        <div class="flex-property flex-center name-time" data-username="%s" data-id="%s">\
                            ...
                            <div class="content %s %s %s" style="color:%s; font-size:%s;"> %s</div>\
                        </div>\
                    </div>',
                ...
                <div class="message flex-property msg-box %s %s">\
                    <img src="%s" alt="%s" class="userItem" >\
                      <div class="flex-property message-info">\
                        ...
                        <div class="content %s %s %s" style="color:%s;font-size:%s;">\
        ...
        if ($chat) {
            ...
            if (chatHTML5.config.linkifyUrl == '1') {
                ...
                templateMessage = templateMessage.replace(regex, function (correspondance, _part1) {
    ...
    this.addUser = function (user, playSound, domUserList, forceDisplay) {
      ...
      <div class="webcamBtn %s"><i class="fa %s fa-2x"></i>%s</div>\
            ...
            user.id, user.username, user.gender, user.hasWebcam, user.status, mutedClass, disableClass, classeFriend, mutedClass, genderStyle,
            user.image, user.username,
            broadcastClass,
        ...
        if (user.broadcast && user.id != chatHTML5.myUser.id) {
            setTimeout(function () {
                ...
                console.log(user.id, user.username, user.broadcast);
            }, 1000);
        }
    ...
    this.talks = function (user) {
        ...
        if (user && user.username) {
            var temp = sprintf('#userList div.userItem[data-username="%s"] i.microphoneTalking', user.username);
            $(temp).show();
        }
        chatHTML5.addWebcam(user.id, user.username, true, true);
    ...
    $('#pushTalkBtn').on('mouseup mouseout', function (_e) {
    ...
    $('#pushTalkBtn').on('mousedown', function (_e) {
    ...
    jQuery.fn.sortDomElements = (function () {
        return function (comparator) {
            return Array.prototype.sort.call(this, comparator).each(function (_i) {
    ...
    $(document).on('click', 'button.raiseHandBtn', function (_e) {
        $(this).remove();
    ...
    $(document).on('click', '.gifAnimated', function (_e) {
        var src = $(this).prop('src');
        $('#smileyContainer').slideToggle(100);
    ...
    $(document).ready(function (_e) {
        chatHTML5.emojiArea = $('#chatInput').emojioneArea({
            ...
            events: {
                paste: function (_editor, _event) {
                    //event.stopImmediatePropagation();
                    //event.preventDefault();
                    //console.log('event:paste');
                },
                'keyup': function (editor, _event) {
                    editor.focus();
                },
                'keydown': function (_editor, event) {
        ...
        $(document).on('autoCompleteUserChoosed', function (_e, data) {
        ...
        $(document).on('keypress', '.windowInputChat', function (e) {
            ...
            if (keyCode === 13) {
                ...
                plainText = chatHTML5.filterBadWord(plainText);
                var extras = { color: $('#colorPicker').val(), date: Date.now(), bold: $('#boldDiv').hasClass('fontSelected'), italic: $('#italicDiv').hasClass('fontSelected'), underline: $('#underlineDiv').hasClass('fontSelected'), bigger: $('#biggerDiv').hasClass('fontSelected') };
                if ((chatHTML5.roles[chatHTML5.myUser.role] || {}).canSendPrivate == '0') {
                    return;
                }
    ...
    $(document).on('click', '.windowEraser', function (_e) {
    ...
    this.displayDateAgo = function () {
        $('#chatContainer .timeStamp').each(function (_index, element) {
            var date = $(element).data('date');
            $(element).text(chatHTML5.getDateAgo(date));
        });
        $('div.windowChat .timeStamp').each(function (_index, element) {
            var date = $(element).data('date');
            $(element).text(chatHTML5.getDateAgo(date));
        });
    };
    ...
    this.removeServerMessages = function () {
        ...
        $('div.serverText').each(function (_index) {
    ...
    $('input#searchInputRoom').keyup(function (_e) {
    ...
    $('input#searchInputRoom2').keyup(function (_e) {
        chatHTML5.searchRooms2();
    });

    this.searchRooms = function () {
        $('#tableRoomBody2 tr').show();
        $('#tableRoomBody2 tr').each(function (_index, element) {
    ...
    this.searchRooms2 = function () {
        ...
        $('#tableRoomBody tr').each(function (_index, element) {
    ...
    this.searchUsers = function () {
        ...
        $('input[data-gender]').each(function (_index, element) {
    ...
    $('.filtergenderItem').click(function (_e) {
        chatHTML5.searchUsers();
    });

    this.loginConference = function (_username, _password) {
    }
    ...
    $(document).on('click', '.registerNow', function (_e) {
        $('#overlayRegister').slideToggle();
    });
    ...
    this.getLastChats = function (maxChats, roomid) {
        ...
        $.post(chatHTML5.config.ajax, { a: 'getChatMessages', maxChats: maxChats, roomid: roomid }, function (chats) {
            ...
            for (var id in chats) {
                ...
                if (chat.message) {
                    var request = 0;
                    chatHTML5.receiveText(user, chat.message, extras, request);
                }
            }
            chatHTML5.displayDateAgo();
    ...
    this.roleShowOnTopofUserList = function (myrole) {
        var res = false;
        $.each(chatHTML5.roles, function (_index, role) {
        ...
        if (!$('div.colorPicker-picker').length) {
            try {
                $('#colorPicker').colorPicker({
                    ...
                    onColorChange: function (_id, newValue) {
    ...
    $('#chatRouletteBtn').click(function () {
        if (chatHTML5.config.chatType == 'roulette') {
            ...
            if (randomUser) {
                chatHTML5.addWebcam(randomUser.id, randomUser.username);
            }
        }
            ...
            if (chatHTML5.config.recordBroadcast == '1' && typeof stopRecordVideo == 'function') {
                ...
                $.post(chatHTML5.config.recordUrl, { filename: chatHTML5.myUser.filename, codec: chatHTML5.config.codec }, function (resultRecord) {
                    ...
                    if (!resultRecord.error) {
                        $.post(chatHTML5.config.ajax, { a: 'saveBroadcast', filename: resultRecord.filename, videourl: resultRecord.videourl, thumburl: resultRecord.thumburl, duration: resultRecord.duration }, function (_res) {
                            resultRecord.userid = chatHTML5.myUser.id;
                            resultRecord.username = chatHTML5.myUser.username;
    ...
    $(document).on('click', '.clickToRegisterBtn', function (_e) {
        $('#overlayRegister').slideToggle();
    });

    this.inviteWebcamChat = function (_id, _username) {
    };
    ...
    this.checkMinorAllowed = function (userid) {
        ...
        try {
            if (chatHTML5.config.minorCanContactAdult == '0' && chatHTML5.isMinor(userid)) {
                bootbox.alert(chatHTML5.traductions.actionForbiddenToMinors, function (_res) {
                });
                return false;
            }
            return res;
    ...
    this.getGenderById = function (genderid) {
        var res = '';
        $.each(chatHTML5.config.genders, function (_id, gender) {
    ...
    this.connectToServer = async function () {
        ...
        this.socket = io.connect(chatHTML5.config.node, { 'force new connection': false, transports: ['websocket'], secure: true, query: chatHTML5.myUser });
        ...
        this.socket.on('rouletteNextUser', function (user, isCaller) {
            if (user) {
                ...
                chatHTML5.addWebcam(user.id, user.username, false);
                if (isCaller) {
        ...
        this.socket.on('call1to1', function (user) {
            ...
            var message = sprintf('\
                ...
                sprintf(chatHTML5.traductions.Incoming1to1From, user.username),
                user.id,
        ...
        this.socket.on('call1to1Refused', function (user) {
            ...
            $('#lightBox1to1').hide();
            $('#myVideo').appendTo(chatHTML5.myUser.webcamContainer);
            bootbox.alert(sprintf(chatHTML5.traductions.call1to1Refused, user.username));
        });

        $(document).on('dblclick', '#myVideo', function () {
            chatHTML5.addWebcamJanus(chatHTML5.myUser.id, chatHTML5.myUser.username, false);
        })
        ...
        this.socket.on('createRoom', function (_room) {
        ...
        this.socket.on('inviteUserPrivateRoom', function (roomid, user) {
            ...
            <div>\
                ...
                roomid, sprintf(chatHTML5.traductions.userWouldLikeToInviteToHisPrivateRoom, user.username, room.name),
                roomid, user.id, user.username, chatHTML5.traductions.accept,
                roomid, user.id, user.username, chatHTML5.traductions.deny,
                roomid, user.id, user.username, chatHTML5.traductions.mute
            );
        ...
        this.socket.on('deleteFriend', function (user) {
            new PNotify({
                ...
                text: sprintf(chatHTML5.traductions.UserIsNotYourFriendAnyMore, user.username),
                type: 'error'
            });
            chatHTML5.getMyFriends();
        });
        ...
        this.socket.on('acceptFriend', function (user) {
            new PNotify({
                ...
                text: sprintf(chatHTML5.traductions.YouAreNowFriendWith, user.username),
                type: 'success'
            });
            chatHTML5.getMyFriends();
        });
        ...
        this.socket.on('refuseFriend', function (user) {
            new PNotify({
                ...
                text: sprintf(chatHTML5.traductions.UserRefusedToBeYourFriend, user.username),
                type: 'error',
                styling: 'bootstrap3'
            });
            chatHTML5.getMyFriends();
        ...
        this.socket.on('refresh', function (_question) {
            chatHTML5.redirectUrl();
        });
        ...
        this.socket.on('removeFriend', function (_friendid) {
        ...
        this.socket.on('deleteUserMessages', function (username) {
            var temp = sprintf("div.message div.userItem[data-username='%s']", username);
        ...
        this.addRoomToFavorite = function (roomid, value) {
            $.post(chatHTML5.config.ajax, { a: 'addRoomToFavorite', roomid: roomid, userid: chatHTML5.myUser.id, webmasterid: chatHTML5.myUser.webmasterid, value: value }, function (_res) {
        ...
        this.getNumberInvisibleUsersInCurrentRoom = function () {
            return (Object.keys(chatHTML5.users).length - $('#userList div[data-username]').length);
        }
        ...
        $('#reportRoomBtn').click(function (_e) {
        ...
        $(document).on('click', 'td i.reportRoomBtn', function (_e) {
        ...
        this.redrawRooms = function (rooms) {
            ...
            for (var index in rooms) {
                ...
                var row = sprintf('\
                <tr data-id="%s" data-name="%s" class="%s" data-adult="%s">\
                    <td>%s %s %s %s %s</td>\
                    <td>%s</td>\
                    <td>%s</td>\
                </tr>',
                    room.name, room.name, trClass, room.isAdult,
        ...
        this.socket.on('maxUsersReached', function (_numberUsersInRoom, _maxUsers) {
        ...
        this.socket.on('kicked', function (user) {
            if (user.username === chatHTML5.myUser.username) {
                ...
                var message = sprintf(chatHTML5.traductions.userHasBeenKicked, user.username);
        ...
        this.socket.on('banned', function (user, minutes, reason) {
            if (user.username === chatHTML5.myUser.username) {
                ...
                var message = sprintf(chatHTML5.traductions.userHasBeenBanned, user.username);
        ...
        this.socket.on('muted', function (user, minutes, description, warnUserOfMute) {
            ...
            if (user.username === chatHTML5.myUser.username) {
                ...
                var message = sprintf(chatHTML5.traductions.userHasBeenMutedBanned, user.username);
        ...
        this.getRoomsIamIn = function () {
            var res = Array();
            $('#chatContainer ul.nav a[data-room="true"]').each(function (_index, el) {
        ...
        this.socket.on('getUsers', function (usersInRoom, _roomid) {
            ...
            if (Object.keys(chatHTML5.users).length && privateChatWith) {
                ...
                if (user) {
                    bootbox.confirm(sprintf(chatHTML5.traductions.areYouSureToCall1to1, user.username), function (res) {
                        if (res) {
                            chatHTML5.sendPrivateInvitation(user.id, user.username);
                        }
                    })
                }
            }
            ...
            var roomid = sprintf('room_%s', chatHTML5.myUser.room.id);
            var roomName = sprintf('<img class="home-tab" src="/img/home.svg"> %s<div class="unread" title="%s"></div>', chatHTML5.myUser.room.name, chatHTML5.traductions.unreadMessages);
            ...
            if (!tab.length) {
                ...
                tabs.addTab({
                    id: roomid, title: roomName, closable: closable, room: true, roomid: chatHTML5.myUser.room.id, label: chatHTML5.myUser.room.name,
        ...
        $('#askPrivateConferenceBtn').click(function () {
            ...
            var message = sprintf(chatHTML5.traductions.AreYouSureToRequestPrivateChatWithModel, performer.username, performer.ppv_pricePerMinute);
        ...
        this.changeUser = function (user) {
            ...
            if (chatHTML5.config.notifyWhenUserStartsWebcam == '1' && user.webcam && !oldWebcam) {
                var message = sprintf(chatHTML5.traductions.XhasTurnedOnHisCam,
                    sprintf("<div class=\"userItem\"  title=\"%s\" data-id=\"%s\" data-username=\"%s\">%s </div>", user.username, user.id, user.username, user.username));
            ...
            if (chatHTML5.config.chatType === 'conference') {
                if (user.role == 'user' && user.webcam) {
                    chatHTML5.serverMessage(sprintf(chatHTML5.traductions.UserOpenHisWebcam, user.username), 'privateClosed');
                }
            }
            // role update
        ...
        $(document).on('click', 'button.createRoomBtn', function (_e) {
        ...
        this.socket.on('webcamAccepted', function (user) {
            chatHTML5.serverMessage(sprintf(chatHTML5.traductions.youAreNowWatchingUser, user.username), 'webcamRequested', user.id);
        ...
        this.socket.on('requestFriend', function (user) {
			...
			<div>\
                ...
                user.username, chatHTML5.traductions.FriendRequested,
                user.id, user.username,
                chatHTML5.traductions.accept,
                user.id, user.username,
                chatHTML5.traductions.deny,
        ...
        this.socket.on('requestPrivateShow', function (user) {
            ...
            var message = sprintf(chatHTML5.traductions.doYouAcceptPrivateShow, user.username);
            bootbox.confirm(message, function (res) {
                if (res) {
        ...
        this.socket.on('inviteWatchCam', function (user) {
            ...
            var invitationMessage = sprintf(chatHTML5.traductions.userInvitedToWatchHisCam, user.username);
            var message = sprintf('\
        ...
        this.socket.on('privateRequested', function (user) {
            ...
            if (chatHTML5.config.privateOnlyOnInvitation == '1') {
                var message = sprintf('\
                    ...
                    user.username, chatHTML5.traductions.requestAPrivateChat,
                    user.id, user.username,
                    chatHTML5.traductions.accept,
                    user.id, user.username,
                    chatHTML5.traductions.deny,
        ...
        this.socket.on('receiveText', function (user, message, extras, request) {
            if (chatHTML5.interval) {
                clearInterval(chatHTML5.interval);
            }
            
            chatHTML5.receiveText(user, message, extras, request);
        ...
        this.socket.on('whisper', function (user, message, extras, request) {
           
            chatHTML5.receiveText(user, message, extras, request);
        });

        this.socket.on('privateDenied', function (user) {
            chatHTML5.serverMessage(sprintf(chatHTML5.traductions.userDeniedPrivateChat, user.username), 'webcamRequested', user.id);
        });
        this.socket.on('watchHisWebcamDenied', function (user) {
            chatHTML5.serverMessage(sprintf(chatHTML5.traductions.userDeniedWatchCam, user.username), 'webcamRequested', user.id);
        });
        ...
        this.socket.on('creditsSent', function (credits, username) {
            var message = sprintf(chatHTML5.traductions.userSentCredits, username, credits);
        ...
        this.socket.on('updateCredits', function (credits, displayInChat, data) {
            ...
            if (data) {
                if (displayInChat) {
                    var message = sprintf(chatHTML5.traductions.userSentYouTips, data.user.username, data.user.credits);
                    chatHTML5.serverMessage(message, 'privateRequested animated flash');
                }
        ...
        this.socket.on('error', function (_errorMessage) {
        ...
        this.socket.on('addUser', function (user) {
            if ((chatHTML5.roles[user.role] || {}).isVisible == '0') {
                ...
                if (chatHTML5.config.displayEnterChatMessage == '1') {
                    var temp = sprintf(chatHTML5.traductions['XhasJoinedTheChat'], user.username);
    ...
    $('#privateWebcamBtn').change(function (_e) {
        chatHTML5.myUser.webcamPublic = false;
        chatHTML5.changeMyStatus();
    });

    $('#publicWebcamBtn').change(function (_e) {
    ...
    this.webrtcAvailable = function (callBackSuccess) {
        ...
        navigator.getUserMedia(mediaOptions, success, function (_e) {
            return false;
        });
    ...
    this.displayMyWebcamKurento = function (value) {
        if (value) {
            chatHTML5.webrtcAvailable(function (_stream) {
    ...
    $(document).on('errorMediaAccess', function (_e) {
    ...
    $(document).on('doNotGetMyStreamId', function (_e) {
        chatHTML5.myUser.streamid = false;
        chatHTML5.myUser.webcam = false;
        chatHTML5.cameraStatus(true);
        $('#webcamBtn').prop('checked', false).change();
    ...
    this.displayMyWebcamMediaSoup = function (value) {
        if (value) {
            if (chatHTML5.config.audioVideo == 'audio') {
                var classe = 'audioOnly';
            } else if (chatHTML5.config.audioVideo == 'video') {
                classe = 'videoOnly';
            } else {
                classe = '';
            }
            ...
            if (chatHTML5.config.audioVideo == 'video') {
                ...
                $(document).ready(function () {
                    ...
                    if (chatHTML5.config.webrtcServer == 'janus') {
                        ...
                        mute('#myVideo', !muted);
                        publishOwnFeed('#myVideo', streamid, $('#videoSelect').val(), (chatHTML5.config.audioVideo == 'video') ? 'none' : $('#audioSelect').val());
                        var webcamOrAudio = ($('#myAudioCheckBox').prop('checked') || $('#myVideoCheckBox').prop('checked'));
                        chatHTML5.myUser.webcam = webcamOrAudio;
                        chatHTML5.cameraStatus(!webcamOrAudio);
                        if (!webcamOrAudio) {
                ...
                document.getElementsByClassName('myAudioCheckBox')[0].className += " btn-default off";
                document.getElementsByClassName('myAudioCheckBox')[0].style.display = "none";

                $('#myAudioCheckBox').prop('checked', false);
            }
    ...
    this.endPrivateShow = function () {
        bootbox.alert(chatHTML5.traductions.privateShowEnded, function (_res) {
    ...
    this.getRoleById = function (roleid) {
        var res = {};
        $.each(chatHTML5.roles, function (_id, role) {
            if (roleid == role.id) {
                res = role;
            }
        });
    ...
    this.publishWebcam = function (_value) {
    ...
    $('#chooseWebcamBtn').click(function () {
        ...
        setTimeout(function () {
            ...
            publishOwnFeed('#myVideo', (chatHTML5.config.audioVideo == 'video') ? 'none' : $('#audioSelect').val(), $('#videoSelect').val());
            $('#webcamChooseModal').modal('hide');
        }, 2000);
    })


    $(document).on('click', '#usersContainer #webcamMinimizeBtn', function (_e) {
        $('#myWebcamDiv').toggleClass('webcamMinimized');
        $('#webcamMinimizeBtn i').toggleClass('fa-minus-square-o').toggleClass('fa-plus-square-o');
    });

    $(document).on('click', '#webcamConfigBtn', function (_e) {
    ...
    this.getWebcamNumber = function () {
        var numberWebcams = 0;
        $('#webcamsContainer').children().each(function (_index) {
            var id = $(this).data('id');
            numberWebcams++
        });
        $('.jsPanel').each(function (_index) {
            if ($(this).find('video')) {
                numberWebcams++
            }
        });
    ...
    $(document).on('click tap touchstart', '.webcamCloseBtn', function (_e) {
    ...
    $(document).on('click', 'div.jsPanel-btn.jsPanel-btn-close', function (_e) {
    ...
    $('#send-msg-btn').click(function (_e) {
        var request = 0;
        chatHTML5.sendText(request);
    });
    ...
    $(document).on('click', '.acceptWatchHisWebcam', function () {
        ...
        $(this).parent().parent().remove();
        if (user) {
            chatHTML5.addWebcam(id, user.username, false, false);
        }
    });
    ...
    this.removeUser = function (user, disconnects) {
        ...
        if (chatHTML5.config.closePrivateChatWhenUserLeaves == '1') {
            ...
            try {
                var texte = sprintf(chatHTML5.traductions.userLeftPrivateChat, user.username);
            } catch (e) {
                texte = '';
            }
        ...
        var leaveMessage = sprintf(chatHTML5.traductions['XhasLeftTheChat'], user.username);
    ...
    $('#clearButton').click(function (_e) {
        bootbox.confirm(chatHTML5.traductions["Confirm clear chat"], function (res) {
            if (res) {
                $('div.tab-content .tab-pane.active').eq(0).empty();
                if(chatHTML5.myUser.role == 'admin' || chatHTML5.myUser.role == 'moderator'){
                    var request = 1;
                    chatHTML5.sendText(request);   
                }
            }
    ...
    this.changeAvatar = async function () {
        ...
        xhr.upload.addEventListener('loadstart', function (_e) {
            $('#progressbar').show();
        }, false);
    ...
    $('#calendarBtn').click(function (_e) {
        $('#calendarContainer').toggle(100);
    });

    $('#clearSearch').click(function (_e) {
        $('#searchInput').val('');
        $('.searchItem').remove();
        chatHTML5.searchUsers();
    });
    ...
    $(document).on('click', '.userItem', async function (event) {
        ...
        if (username === undefined) {
            username = $(this).parent().find('[data-username]').data('username');
            id = $(this).parent().find('[data-username]').data('id');
        }
        if (id == chatHTML5.myUser.id || username == chatHTML5.myUser.username) {
            return;
        }
        ...
        if (chatHTML5.getRoomsIOweNumber()) {
            ...
            var myRooms = chatHTML5.getRoomsIOwe();
            $.each(myRooms, function (_roomid, myroom) {
    ...
    $('#biggerDiv').click(function () {
        $(this).toggleClass('fontSelected');
        document.getElementsByClassName('dropup-content')[0].style.display = (document.getElementById('biggerDiv').className.includes('fontSelected') )?'block':'none';
    }) 
    $('.fontsize-select').click(function () {
        font_size = $(this).text();
        font_size = font_size + 'px'
        console.log(font_size);
        document.getElementsByClassName('dropup-content')[0].style.display = 'none';
        chatHTML5.changeFontsize(font_size);
    ...
    $('#mutePrisonBtn').click(function (_e) {
    ...
    this.mute = function (id, _username) {
    ...
    this.showCall1to1LightBox = function (user, playMP3, endtimeOut) {
        ...
        $("#myVideo")[0].play();
        $('#username1to1Call').text(sprintf(chatHTML5.traductions.callingUser, user.username));
        if (playMP3) {
            chatHTML5.playMP3(chatHTML5.config.waitPhoneMP3);
        }
        chatHTML5.myUser.calling1to1User = user;
        $('username1to1Call').text(sprintf(chatHTML5.traductions.callingUser), user.username).show();
    ...
    this.call1to1 = function (userid) {
        ...
        chatHTML5.myUser.webcamContainer = $('#myVideo').parent();
        var user = (chatHTML5.users[userid] || {});
    ...
    this.call1to1Ended = function (user, timeOut) {
        ...
        clearInterval(chatHTML5.myUser.calling1to1Interval);
        if (timeOut) {
            bootbox.alert(sprintf(chatHTML5.traductions.didNotAnswer, user.username));
            return;
        } else {
            bootbox.alert(sprintf(chatHTML5.traductions.call1to1Ended, user.username));
        }
    ...
    $('#userMenu, ul.dropdown-menu').on('click', '.menuUserItem', async function (_e) {
        ...
        switch (action) {
            ...
            case 'reportUser':
                chatHTML5.reportUsername = $(this).parent().data('username');
    ...
    this.reportUser = function (usernameProblem, email, description, reportReason) {
        bootbox.confirm(chatHTML5.traductions.confirm, function (res) {
            ...
            $.post(chatHTML5.config.ajax, { a: 'report', usernameProblem: usernameProblem, usernameAuthor: chatHTML5.myUser.username, email: email, description: description, reportReason: reportReason }, function (_res) {
                bootbox.alert(chatHTML5.traductions['Report Send to Admin']);
    ...
    $('#quitBtn, #customExit').click(function (_e) {
    ...
    $('#tabs').on('click', 'button.btnYoutubeCloseMessage', function (_e) {
        $(this).parent().parent().remove();
    })
...
$(document).on('tabClosed', function (_e, tab) {
    if (tab.room) {
...
$.fn.ulSelect = function () {
    ...
    $(this).on('click', 'li', function (_event) {
...
$('body').on('contextmenu', 'img', function (_e) {
    return false;
});
