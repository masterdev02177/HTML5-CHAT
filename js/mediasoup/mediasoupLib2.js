...
function connectMS(SOCKETIO_SERVER, _streamid) {
    myStreamid = _streamid;
    // Connect to the socketMediaSoup.io pubsub server (../server.js), and update status.
    if (socketMediaSoup.connected) {
        socketMediaSoup.close();
    }
    socketMediaSoup = io.connect(SOCKETIO_SERVER);
...
function publishOwnFeed(_streamid) {
    myStreamid = _streamid;
    if (pubsession) {
        // We can just delete the room, since our MS_STOP message will
        // properly be handled by MediaSoup and clean up the rest of the client.
        delete roomsMS[pubsession];

        // Don't expect an error or response.
        sendMS({type: 'MS_STOP', payload: {}, meta: {id: ++reqid, session: pubsession}});
    }
    // Mark us as not currently publishing.
    pubsession = undefined;
