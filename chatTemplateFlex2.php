...
<head>
    ...
    <base href="https://server2.buychatroom.com/">
    ...
    <link rel="stylesheet" type="text/css" href="css/chatHTML5.css?rnd=<?=time()?>">
    <link rel="stylesheet" type="text/css" href="css/<?=$config->theme?>Chat4.css?rnd=<?=time()?>">

    <?php if ($config->externalCSSLink && filter_var($config->externalCSSLink, FILTER_VALIDATE_URL) !== false): ?>
        <link rel="stylesheet" type="text/css" href="<?=$config->externalCSSLink?>?rnd=<?=time()?>">
    <?php endif?>
    ...
    <script src="js/bootstrap-dynamic-tabs.js?rnd=<?=time()?>"></script>
    <script src="js/perfect-scrollbar.jquery.js" async></script>
    <!-- <script src="https://www.youtube.com/iframe_api" async></script> -->


    <script src="https://cdnjs.cloudflare.com/ajax/libs/jspanel3/3.11.2/jquery.jspanel.min.js"></script>
    <script src="https://cdn.jsdelivr.net/npm/jquery.marquee@1.5.0/jquery.marquee.js" type="text/javascript"></script>

    <script src="js/emojionearea.js?rnd=<?=time()?>"></script>
    <script src="js/jquery.textcomplete.js?rnd<?=time()?>"></script>
    ...
    <?php if ($config->displayCalendar): ?>
        ...
        <link rel="stylesheet" type="text/css" href="css/zabuto_calendar.min.css?rnd=<?=time()?>">
    <?php endif?>
    ...
    <?php if ($config->webrtc && $config->webrtcServer=='kurento'): ?>
        ...
        <script src="https://webtv.fr:8080/socket.io/socket.io.js"></script>
        <script src="js/kurento.js?rnd=<?=time()?>"></script>
        <script>
            var serverWebrtc = {};
        </script>
    <?php endif?>
    ...
    <?php endif ?>
    <link rel="stylesheet" href="css/flex.css?rnd=<?=time()?>">
    <style>
        <?=$config->css?>
    </style>
...
<div id="fullMainContainer">
    <div id="header" class="flex-property">
        ...
        <div class="dropdown ham-dropdown-menu">
            <button class="dropdown-toggle hamburger-header flex-property" type="button" data-toggle="dropdown">
                <div></div>
                <div></div>
                <div></div>
            </button>
        ...
        <div class="flex-property group-btns header-custom-btns">
            ...
            <?php if ($config->conferencePrivatePaymentUrl && !$roles[$myuser['role']]->canBroadcast): ?>
            ...
            <?php if ($config->displayRoomButton): ?>
                <button id="roomsBtn" class="header-home-btn flex-property"><img class="svg-header-icons" src="img/home.svg"/><?=$traductions['roomList'];?></button>
            <?php endif?>
            ...
            <?php if ($config->displayWebcamButton || ($config->chatType=='conference' && $roles[$myuser['role']]->conference_canUsersShowWebcamInPublic)): ?>
                <div class="header-btn-wrap flex-property">
                    <span class="buttons-subnames flex-property"><img class="svg-header-icons" src="img/webcam.svg" /><span><?=$traductions['Camera']?></span></span>
            ...
            <div class="header-btn-wrap flex-property" title="<?=$traductions['do not play sound effects'];?>">
            ...
            <div id="chatContainer">
                ...
                <div id="footer" class="flex-property">
                    <div class="textarea-icons-wrapper flex-property">
                        ...
                        <div class="chat-input-icons" id="boldDiv">
                            <i class="fa fa-bold"></i>
                        </div>
                        <div class="chat-input-icons" id="italicDiv">
                            <i class="fa fa-italic"></i>
                        </div>
                        <div class="chat-input-icons" id="underlineDiv">
                        ...
                        <?php if ($roles[$myuser['role']]->colorPicker):?>
                            <div class="chat-input-icons" id="colorPickerContainer"><input id="colorPicker" name="colorPicker" type="text" /></div>
                        <?php endif?>
                    ...
                    <div id="usersContainer2" class="tab-pane active">
                        <?php if ($roles[$myuser['role']]->canSeeOtherUsers):?>
                            <div class="flex-property search-bar">
                                ...
                                <button id="sortBtn" data-sort="1" class="btn btn-sm btn-default" > <i class="fa fa-sort-alpha-asc" aria-hidden="true"></i></button>
                            </div>
                        <?php endif?>
                                ...
                                <div id="myWebcamContainer">
                                    ...
                                    <?php if ($config->chatroulette): ?>
                                        <div id="chatrouletteContainer">
                                            <button id="chatRouletteBtn" class="btn btn-success active "><i class="fa fa-random" aria-hidden="true"></i> <?=$traductions['playRandomCam'];?></button>
                                        </div>
                                    <?php endif?>
...
<?php if ($config->webrtc && $config->webrtcServer=='licode'): ?>
    ...
    <script src="js/licode/licode.js?rdn=<?=time()?>"></script>
<?php endif?>
...
<?php if ($config->webrtc && $config->webrtcServer=='janus'): ?>
    ...
    <script type="text/javascript" src="js/janus/janus.js?id=<?=time()?>" ></script>
    <script type="text/javascript" src="js/janus/webrtc.js?id=<?=time()?>"></script>
<?php endif?>

<?php if ($config->webrtc && $config->webrtcServer=='mediasoup'): ?>
    <script type="text/javascript" src="js/mediasoup/mediasoup-client.js" ></script>
<?php endif?>

<?php if ($config->webrtc && $config->webrtcServer=='mediasoup'): ?>
    <script type="text/javascript" src="js/mediasoup/client.js?cache=<?=time()?>"></script>
<?php endif?>
